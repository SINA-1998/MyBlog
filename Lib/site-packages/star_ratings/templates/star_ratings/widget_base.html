...
<div id="{{ id }}" class="star-ratings" data-max-rating="{{ star_count }}" data-avg-rating="{{ rating.average }}">
            ...
            <ul class="star-ratings-rating-background">
                {% for star in stars %}
                    <li>
                        ...
                        {% elif user.is_authenticated or read_only %}
    ...
    <div class="star-ratings-rating-average">{% trans 'میانگین امتیاز: ' %} <span class='star-ratings-rating-value'>{{ rating.average|floatformat:"-2" }}</span></div>
    {% endblock rating_average %}
    {% block rating_count %}
    <div class="star-ratings-rating-count">{% trans 'تعداد رای: ' %} <span class='star-ratings-rating-value'>{{ rating.count }}</span></div>
    {% endblock rating_count %}
    {% block rating_user %}
    <div class="star-ratings-rating-user">{% trans 'امتیاز شما: ' %} <span class='star-ratings-rating-value'>{% if user_rating %}{{ user_rating.score }}{% else %}{% trans 'رای ثبت نشده است' %}{% endif %}</span></div>
    {% if not user.is_authenticated and not anonymous_ratings and not read_only %}
        <p>{% trans 'لطفا برای رای دادن وارد شوید.' %}</p>
    {% endif %}
